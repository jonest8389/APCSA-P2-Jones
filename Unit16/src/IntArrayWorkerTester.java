public class IntArrayWorkerTester
{
  ...
  {
    IntArrayWorker worker = new IntArrayWorker();
    int [][] nums2 = {{1, 2, 3}, {4, 5, 6}, {2, 3, 4}};
    worker.setMatrix(nums2);
    int total = worker.getColTotal(0);
    System.out.println("Total for column 0 should be 6 and is " + total);
    total = worker.getColTotal(1);
    System.out.println("Total for column 1 should be 15 and is " + total);
    total = worker.getColTotal(2);
    System.out.println("Total for column 2 should be 9 and is " + total);
    // Student's note: This tester was broken so I modified it to make it work
  }
